...
CREATE UNIQUE INDEX ux_users_username
  ON t_sc_users (username);
CREATE UNIQUE INDEX ux_users_mobile
  ON t_sc_users (mobile);

--  角色表
DROP TABLE IF EXISTS t_sc_roles;
CREATE TABLE t_sc_roles
(
...
-- 资源表
DROP TABLE IF EXISTS t_sc_resource;
CREATE TABLE t_sc_resource
(
...
CREATE UNIQUE INDEX ux_resource_code
  ON t_sc_resource (code);

-- 用户和角色关系表
DROP TABLE IF EXISTS t_sc_user_role_relation;
CREATE TABLE t_sc_user_role_relation
(
...
-- 角色和资源关系表
DROP TABLE IF EXISTS t_sc_role_resource_relation;
CREATE TABLE t_sc_role_resource_relation
(
  id           VARCHAR(20) PRIMARY KEY COMMENT '关系id',
